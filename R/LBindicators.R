rm(list = ls())
# LBindicators.R
# T. Miethe and C. Silva, WKLIFE-V, Oct2015

# Fill in life history parameters, sexes, start and end year
# Choose bin size (ClassInt) width for estimation of Lc
# Run indicator calculation and plots
...
Linf <- c(70,65)
Lmat<-c(28.4, 30)
# S <- c("M","F","N")  # sexes for which analyses have to be performed on, M males, F females, N unsexed
S <- c("M","F")

###############################################################################################
## part I - script to load data in Intercatch format for one country (for compiled data in tables go to part II)

ctype <- "L"  # for landings L, discard D
ftype <- ".dat"
file_path <- paste0("ICES_MSY/data/", stock, "/")

# step3 load first year data, Intercatch SD file
icdata <- read.table(paste(file_path, country,"_",startyear,"_DataCall2015_DLS_",stock,ftype,sep = ""),sep = ",",header = FALSE, col.names = as.character(1:33), fill = T)
...
if(icdata$unitMeanWeight[1]=="g") multiplier <- 0.001 else multiplier <- 1   # adapt MeanWeight multiplier

# str(icdata$NumberCaught)
# unique(icdata$unitCanum) #check format of NumberCaught numeric, in thousands k
...
for(year in Year1){
  for(i in 1:length(S)){
    ...
    icdata <- read.table(paste(country,"_",year,"_DataCall2015_DLS_",stock,ftype,sep=""),sep=",",header=FALSE, col.names=as.character(1:33), fill=T)
    colnames(icdata) <- c("RecordType","Country", "Year", "SeasonType", "Season", "Fleet", "AreaType", "FishingArea","DepthRange","Species","Stock","CatchCategory","ReportingCategory","Sex", "CANUMtype", "AgeLength", "PlusGroup","sampledCatch", "NumSamplesLngt","NumLngtMeas","NumSamplesAge","NumAgeMeas","unitMeanWeight","unitCanum", "UnitAgeOrLength","UnitMeanLength","Maturity","NumberCaught","MeanWeight","MeanLength", "varNumLanded","varWgtLanded", "varLgtLanded")
    icdata <- subset(icdata, RecordType=='SD' & CatchCategory==ctype)
    
    icdatanum <- icdata[icdata$Sex==sex,c("MeanLength","NumberCaught")]
    colnames(icdatanum) <- c("MeanLength",year)
    
    icdataw <- icdata[icdata$Sex==sex,c("MeanLength","MeanWeight")]
...
######################################################################################################
## part II - Load data Numbers-at-length per year as a dataframe with the following format: 
...
# !!!!!!!!! Only Run this block if the MeanLength is not the class midpoint but the class lower bound and bin size 1

if(FALSE){
  m$MeanLength <- m$MeanLength+.5
  f$MeanLength <- f$MeanLength+.5
  ns$MeanLength <- ns$MeanLength+.5
}
...
df0 <- m                 # Insert f for Females, m for Males or ns for Unsexed
sexplot <- "M"           # M male data set for length frequ plot

# save the original plot
  df0.long <- melt(df0, id.vars='MeanLength')
  df0.long$variable <- gsub("X", "", as.character(df0.long$variable))
  
  png(paste0(stock,"_",sexplot,"length_dist.png"), bg="white", pointsize=5,units="cm", width=35, height=18, res = 600)
  barchart(value~MeanLength|variable, data=df0.long, horizontal=F, as.table=T, ylim=c(0,NA), xlab='Length (mm)', ylab='Number (10^3)', scales=list(x=list(at=seq(2,length(unique(df0.long$MeanLength)),8), labels=seq(min(df0.long$MeanLength)+1,max(df0.long$MeanLength),8))), main=stock )
  dev.off()

#regrouping
  ClassInt <- 2  # 2mm length class - it can be changed!
...
#regrouped plot
  barchart(Number~lmidp|as.factor(year), data=df0.gr.long, horizontal=F, as.table=T, ylim=c(0,NA), xlab='Length', ylab='Number (10^3)', scales=list(x=list(at=seq(2,length(unique(df0.gr.long$lmidp)),4), labels=seq(min(df0.gr.long$lmidp)+ClassInt,max(df0.gr.long$lmidp),4*ClassInt))), main=stock, cex.main=1.2)
...
for(s in 1:length(S)){
 sex <- S[s] 
  ...
  write.csv(Ind, file=paste(stock,"_",sex,"_IndicatorRatios_table.csv",sep=""), row.names=F)
...
png(paste(stock,"_",sex,"_timeseries.png",sep=""), bg="white", pointsize=5,units="cm", width=10, height=18, res = 600)

  par(mar = c(5, 4, 3, 4), mfrow=c(3,1), family="serif", cex=1.5)
  
    plot(Linf~Year, data=Ind, ylab="Length", col="transparent", main="(a) Conservation", xlab="Year", xlim=c(Year[1],tail(Year,1)+1), ylim=c(min(Ind$Lc)*.9, unique(Ind$Linf)*1.1), bty="l")
...
png(paste(stock,"_",sex,"_timeseries_ratios.png",sep=""), bg="white", pointsize=5, units="cm", width=10, height=18, res = 600)
