# return_LL - function that calculates the negative log likelihood
#         given mean length data and mortality parameters (with analytical solution for sigma)

# return_LL_eq - function that calculates the negative log likelihood
#         given mean length data and mortality parameters (with analytical solution for sigma)
#         This function assumes zero changes in mortality.

# gh - function that wraps up the optimization and returns estimates of
#      Z, years of change, with standard errors and sigma  and AIC value
...
return_LL <- function(x, dat, nbreaks, K, Linf, Lc) {
    ...
    for(i in 1:(nbreaks + 1)) {
      ...
      if (i <= nbreaks) {
        denom[m] <- denom[m] + a[i, m] * 
          ((1 - exp(-Z[nbreaks + 2 - i] * dy[nbreaks + 1 - i, m]))/(1 - exp(-Z[nbreaks + 2 - i])))
      }
      
      if (i == (nbreaks + 1)) {
        denom[m] <- denom[m] + a[i, m]/(1 - exp(-Z[nbreaks + 2 - i]))
      }
      
      numersum[m] <- numersum[m] + r[i, m] * s[i, m]/(1-exp(-Z[nbreaks + 2 - i] - K))
    }
  }
  ...
  sig <<- sqrt(sum(ss * (Lbar - Lpred)^2, na.rm = TRUE)/nyr)
  
  LL <- -nyr * log(sig) - sum(ss * (Lbar - Lpred)^2, na.rm = TRUE)/(2 * sig^2)
  return(-1 * LL)
}
...
return_LL_eq <- function(x, dat, K, Linf, Lc) {
  ...
  Lpred <<- rep(Linf * (1 - ((1-exp(-Z))/(1-exp(-Z-K)))*(1 - Lc/Linf)), nyr)
  sig <<- sqrt(sum(ss * (Lbar - Lpred)^2, na.rm = TRUE)/nyr)
...
gh <- function(year = NULL, mlen = NULL, ss = NULL, K = NULL, Linf = NULL, 
  ...
  if(nbreaks == 0) {
    tab <- data.frame(Parameter = c("Z", "sigma", "AIC"), 
                      Estimate = c(results$par, sig, AIC),
                      StdErr = c(SE, NA, NA))
  }
  else {
    ...
    tab <- data.frame(Parameter = vname, Estimate = c(est, sig, AIC), StdErr = c(SE, NA, NA))
  }
  
  if(graph) {
    par(mfrow = c(1, 2))
    plot(year, mlen, xlab = "Year", ylab = "Mean Length", typ = "o", pch = 16)
    lines(year, Lpred, col = "red")
    plot(year, mlen - Lpred, xlab = "Year", ylab = "Residual", typ = "o", pch = 16)
    abline(h = 0, col = "red")
  }
  ...
  return(list(summary = tab, convergence = status, hessian = hess.status, 
              mlen = data.frame(year = year, observed = mlen, predicted = Lpred, residual = mlen - Lpred)))
